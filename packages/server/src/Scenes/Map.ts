...
import { TiledMap, isTiledFormat } from "@rpgjs/tiled";
...
import { MapData, MapOptions } from "../decorators/map";
...
export class SceneMap {
  ...
  getMapBydId(id: string): RpgClassMap<RpgMap> | null {
    ...
    if (!Utils.isClass(mapClass))
      mapClass = Utils.createConstructor<RpgClassMap<RpgMap>>(mapClass);
    return mapClass;
  }
   ...
   */
  createDynamicMap(
    mapData: MapOptions | string | RpgClassMap<RpgMap> | TiledMap
  ): RpgClassMap<RpgMap> | never {
    if (Utils.isString(mapData)) {
      const id = Utils.extractId(mapData as string);
      if (!id) {
        throw new Error(
          "Unable to extract the file identifier. Check that the file has only the following characters: [a-zA-Z0-9-_$!]+"
        );
      }
      mapData = {
        id: id[1],
    ...
    if (!(mapData as MapOptions).id)
      (mapData as MapOptions).id = Utils.generateUID();
  ...
  async changeMap(
    ...
    const boolArray: boolean[] = await RpgPlugin.emit(
      HookServer.PlayerCanChangeMap,
      [player, this.getMapBydId(mapId)],
      true
    );
    ...
    const scalabilityArray: boolean[] = await RpgPlugin.emit(
      HookServer.ScalabilityChangeServer,
      player
    );
    ...
    await mapInstance.physicScene.addObject(player, {
      x: 0,
      y: 0,
      width: mapInstance.tileWidth,
      height: mapInstance.tileHeight,
    });
