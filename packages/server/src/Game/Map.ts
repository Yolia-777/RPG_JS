...
export class RpgMap extends RpgCommonMap {
  ...
  async load() {
    ...
    super.load(data);
    await this.physicScene.createWorker();
    ...
    await this.createDynamicEvent(this._events as EventPosOption[]);
  ...
  remove(ignorePlayers = false): never | void {
    ...
    if (players.length > 0 && !ignorePlayers) {
      throw new Error(
        `Cannot remove map ${this.id} because there are still players on it`
      );
    }
  ...
  private async parseTmx(file: string, relativePath: string = "") {
    ...
    const parser = new TiledParserFile(file, {
      ...
      staticDir: hasAssetsPath
        ? path.join(this._server.inputOptions.basePath, this._server.assetsPath)
        : "",
      relativePath,
    });
  ...
  private loadProperties(properties: { [key: string]: any }) {
  ...
  // @internal
  loadCommonEvents(
    commonEvents: RpgClassEvent<RpgEvent>[],
    player?: RpgPlayer
  ) {
  ...
  setTile(
    x: number,
    y: number,
    layerFilter: string | ((layer: any) => boolean),
    tileInfo: {
      gid: number;
      properties?: object;
    }
  ...
  async createDynamicEvent(
    eventsList: EventPosOption | EventPosOption[]
  ): Promise<{
    [eventId: string]: RpgEvent;
  }> {
  ...
  async createEvent(
    ...
    const ev = await this.game.addEvent<RpgEvent>(event);
  ...
  async createEvents(
    ...
    for (let obj of eventsList) {
      const ev = await this.createEvent(obj as EventPosOption, mode);
      if (ev) {
        events[ev.id] = ev;
      }
    }
...
Utils.applyMixins(RpgMap, [EventManager]);
